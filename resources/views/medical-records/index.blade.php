...
<head>
    ...
    <style>
        ...
        .dashboard-bg {
            background: linear-gradient(rgba(255,255,255,0.78), rgba(255,255,255,0.78)), url('{{ asset('images/medical-hexagons.jpg') }}') center center / cover no-repeat;
...
<body class="bg-dashboard">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    @if(auth()->check() && auth()->user()->isDoctor())
                        {{-- Doctor: Hide About, Health Tips, Contact --}}
                    @else
                    ...
                    @endif
                    @if(auth()->check() && auth()->user()->isDoctor())
                        {{-- Doctor: Hide Appointments --}}
                    @else
                        <li class="nav-item"><a class="nav-link" href="{{ route('appointments.index') }}">Appointments</a></li>
                    @endif
                    <li class="nav-item"><a class="nav-link active" href="{{ route('patient.medical-records.index') }}">Medical Records</a></li>
                    @auth
    ...
    <!-- Doctor: Search by Patient ID -->
    @if(auth()->check() && auth()->user()->isDoctor())
    <div class="container my-4">
        <form method="GET" action="{{ route('doctor.medical-records.index') }}" class="row g-3 justify-content-center">
            <div class="col-auto">
                <input type="number" name="patient_id" class="form-control" placeholder="Search by Patient ID" value="{{ request('patient_id') }}">
            </div>
            <div class="col-auto">
