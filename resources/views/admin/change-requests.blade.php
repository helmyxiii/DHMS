    ...
    <table class="table">
        ...
        @foreach($requests as $req)
            <tr>
                ...
                <td>{{ $req->appointment->appointment_date ?? '' }} ({{ $req->appointment->doctor->name ?? ($req->appointment->doctor->user->name ?? 'Unknown') }})</td>
                <td>{{ ucfirst($req->type) }}</td>
                <td>{{ $req->message }}</td>
                <td>{{ ucfirst($req->status) }}</td>
                <td>
                    @if($req->status === 'pending')
                    <form method="POST" action="{{ route('admin.changeRequests.process', $req) }}" style="display:inline;">
                        @csrf
